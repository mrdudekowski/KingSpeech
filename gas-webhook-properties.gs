...
function getConfig() {
  ...
  return {
    ...
    MANAGER_CHAT_ID: properties.getProperty('MANAGER_CHAT_ID') || 'YOUR_CHAT_ID',
    
    // Google Sheets настройки
    SPREADSHEET_ID: properties.getProperty('SPREADSHEET_ID') || 'YOUR_SPREADSHEET_ID',
    SHEET_NAME: properties.getProperty('SHEET_NAME') || 'LEADS',
...
function doPost(e) {
  try {
    ...
    const sheetsResult = saveToSheets(formData, CONFIG);
 ...
 */
function saveToSheets(data, CONFIG) {
  try {
    ...
    // Форматируем данные
    const formattedData = formatDataForSheets(data, CONFIG);
 ...
 */
function formatDataForSheets(data, CONFIG) {
  const now = new Date();
...
function testConfiguration() {
  ...
  // Тест Sheets
  const sheetsTest = getStats(CONFIG);
  if (sheetsTest.error) {
